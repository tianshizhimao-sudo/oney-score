...
(function () {
  ...
  /* Backward-compat shim for report URLs generated by the previous
     deployment (flat payload shape). Detect absence of `report`
     sub-object and reshape to the current contract so bookmarked
     links keep rendering. New shapes pass through unchanged. */
  function migrateLegacyPayload(raw) {
    if (!raw || typeof raw !== 'object') return raw;
    if (raw.report) return raw; // already the new shape

    // Legacy: top-level flat payload. Project into { report, lead, share }.
    if (raw.report_id == null && raw.overall_score == null) return raw; // not legacy either
    var legacyDims = Array.isArray(raw.dimensions) ? raw.dimensions : [];
    var dimMap = {};
    legacyDims.forEach(function (d) { if (d && d.id != null) dimMap[d.id] = d.score; });

    var legacyTags = (raw.insight && Array.isArray(raw.insight.profile_tags)) ? raw.insight.profile_tags : [];
    var tagLabels = legacyTags.map(function (t) { return (t && t.label) || t; }).filter(Boolean);

    var legacyTop = Array.isArray(raw.top_actions) ? raw.top_actions : [];
    var bandMap = { 'needs-work': 'needs_work' };
    var band = bandMap[raw.readiness_band] || raw.readiness_band;
    var pathFromBand = band === 'strong' ? 'approach_bank'
                     : band === 'borderline' ? 'broker_review' : 'improve_first';
    ...
    return {
      report: {
        ...
        readiness_band:           band,
        recommended_path:         pathFromBand,
        dimension_scores:         dimMap,
        insight_completion_state: (raw.insight && raw.insight.completion_state) || 'skipped',
        profile_tags:             tagLabels,
        profile_summary:          (raw.insight && raw.insight.profile_summary) || null,
        top_priority_actions:     legacyTop.map(function (a) { return { id: a.id, label: a.label, body: a.text || a.body || '' }; }),
        product_version:          raw.product_version || '—',
        scoring_version:          raw.scoring_version || '—',
        disclaimer_version:       raw.disclaimer_version || '—'
      },
      lead: {
        first_name:    (raw.lead && raw.lead.first_name) || '',
        business_name: (raw.lead && raw.lead.business_name) || ''
      },
      share: (raw.lead && raw.lead.share && raw.lead.share.enabled)
        ? { enabled: true, recipient_type: 'broker' }
        : { enabled: false }
    };
  }
  ...
  function getHashPayload() {
    ...
    for (var i = 0; i < parts.length; i++) {
      var p = parts[i];
      if (p.indexOf('r=') === 0) {
        var decoded = window.OneyReportPlatform.decodePayloadFromUrl(p.slice(2));
        return migrateLegacyPayload(decoded);
      }
    }
    return null;
  }
